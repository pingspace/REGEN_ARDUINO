...
#define CMD_BYTE 0x67
#define CMD_INTERVAL 1000

static uint16_t crc16_checksum(const unsigned char *buf, unsigned int len);

// Pointer to data structure
regen_send_cmd_st data;

void prepare_protocol_data(uint8_t sensor_1, uint8_t sensor_2)
{

  data.header.head_byte = HEADER_BYTE;
  data.header.cmd_byte = CMD_BYTE;
  data.header.data_len = 0x20;
  data.sensor_1 = sensor_1;
  data.sensor_2 = sensor_2;
  data.crc_checksum = crc16_checksum((const unsigned char *)&data.header.data_len, sizeof(regen_send_cmd_st) - 3);
}

void sensor_data()
{
  prepare_protocol_data(3, 4);
}
...
void loop()
{
  ...
  Serial.write((byte *)&data, sizeof(regen_send_cmd_st));
